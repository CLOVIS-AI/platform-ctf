...
source "vsphere-iso" "autogenerated_1" {
  ...
  boot_command         = ["root<enter><wait>", "ifconfig eth0 up && udhcpc -i eth0<enter><wait5>", "mount -t vfat /dev/fd0 /media/floppy<enter><wait>", "printf \"${var.generic_password}\\n${var.generic_password}\\ny\\n\" | setup-alpine -f /media/floppy/answerfile ; reboot<enter>", "<wait120>", "${var.generic_user}<enter>", "${var.generic_password}<enter><wait>", "mount -t vfat /dev/fd0 /media/floppy<enter><wait>", "/media/floppy/SETUP.SH<enter>"]
  boot_wait            = "15s"
