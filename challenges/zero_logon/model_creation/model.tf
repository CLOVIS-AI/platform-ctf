...
#
# Le nouveau dossier créé pour le scénario
#
resource "vsphere_folder" "folder" {
  path          = "plateforme-rsr/scenarios/${var.scenario_name}"
  type          = "vm"
  datacenter_id = data.vsphere_datacenter.dc.id
}
...
resource "vsphere_host_virtual_switch" "sw_demo_LAN" {
  name           = "SW_${var.scenario_name}_LAN"
...
resource "vsphere_host_port_group" "net_demo_LAN" {
  name                = "SW_${var.scenario_name}_LAN"
  host_system_id      = data.vsphere_host.host.id
  virtual_switch_name = vsphere_host_virtual_switch.sw_demo_LAN.name
}
data "vsphere_network" "net2_demo_LAN" {
  name          = "SW_${var.scenario_name}_LAN"
  datacenter_id = data.vsphere_datacenter.dc.id
  depends_on = [
    vsphere_host_port_group.net_demo_LAN,
  ]
}
...
# Template
data "vsphere_virtual_machine" "template_model_demo_pfsense" {
  name          = "generic_pfsense"
  datacenter_id = data.vsphere_datacenter.dc.id
}
...
resource "vsphere_virtual_machine" "vm_pfsense" {
  ...
  num_cpus = data.vsphere_virtual_machine.template_model_demo_pfsense.num_cpus
  memory   = data.vsphere_virtual_machine.template_model_demo_pfsense.memory
  guest_id = data.vsphere_virtual_machine.template_model_demo_pfsense.guest_id
  scsi_type = data.vsphere_virtual_machine.template_model_demo_pfsense.scsi_type

  # Première interface (WAN)
  network_interface {
    network_id   = data.vsphere_network.vm_network.id
    adapter_type = data.vsphere_virtual_machine.template_model_demo_pfsense.network_interface_types[0]
  }

  # Deuxième interface (LAN)
  network_interface {
    network_id   = data.vsphere_network.net2_demo_LAN.id
    adapter_type = data.vsphere_virtual_machine.template_model_demo_pfsense.network_interface_types[1]
  }

  disk {
    label            = "disk0"
    size             = data.vsphere_virtual_machine.template_model_demo_pfsense.disks.0.size
    eagerly_scrub    = data.vsphere_virtual_machine.template_model_demo_pfsense.disks.0.eagerly_scrub
    thin_provisioned = data.vsphere_virtual_machine.template_model_demo_pfsense.disks.0.thin_provisioned
  }

  # Clone lié pour économiser de la place
  clone {
    template_uuid = data.vsphere_virtual_machine.template_model_demo_pfsense.id
    linked_clone = false
  }
...
data "vsphere_virtual_machine" "template_model_windows_10" {
  name          = "generic_windows_10_enterprise"
  datacenter_id = data.vsphere_datacenter.dc.id
}
...
data "vsphere_virtual_machine" "template_model_windows_server" {
  name          = "generic_windows_server_2019"
  datacenter_id = data.vsphere_datacenter.dc.id
}
...
output "Windows_10_Ip" {
  value = vsphere_virtual_machine.vm_windows_10.default_ip_address
  description = "IP du client Windows"
}

output "Windows_Server_Ip" {
  value = vsphere_virtual_machine.vm_windows_server.default_ip_address
  description = "IP du serveur Windows"
}

output "Kali_Ip" {
  value = vsphere_virtual_machine.vm_kali.default_ip_address
  description = "IP du client Kali"
}
